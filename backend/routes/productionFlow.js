...
const LOT_ASSIGNMENT_STAGES = new Set(['back_pocket', 'stitching_master']);
...
router.get(
  '/production-flow/lots/:lotNumber',
  isAuthenticated,
  allowRoles(ALLOWED_ROLES),
  async (req, res) => {
    const user = req.session.user;
    ...
    if (!LOT_ASSIGNMENT_STAGES.has(stage)) {
      return res.status(403).json({ error: 'Lot preview is only available for back pocket or stitching stages.' });
    }

    const lotNumber = normaliseCode(req.params.lotNumber);
    if (!lotNumber) {
    ...
    const connection = await pool.getConnection();
    try {
      const lot = await fetchLotWithSizes(connection, lotNumber);
      if (!lot) {
      ...
      for (const row of eventRows) {
        ...
        eventByBundleId.set(row.bundle_id, row);
        assignedBundles += 1;
        if (row.is_closed) {
          closedBundles += 1;
        } else {
      ...
      for (const size of lot.sizes) {
        ...
        for (const bundle of size.bundles) {
          totalBundles += 1;
          const event = eventByBundleId.get(bundle.bundle_id) || null;
          const isAssigned = Boolean(event);
          const isClosed = isAssigned ? Boolean(event.is_closed) : false;
          if (isAssigned) {
            sizeAssigned += 1;
            if (isClosed) {
              sizeClosed += 1;
          ...
          bundles.push({
            bundleId: bundle.bundle_id,
            bundleCode: bundle.bundle_code,
            piecesInBundle: bundle.pieces_in_bundle,
            assigned: isAssigned,
            isClosed,
            masterId: event ? event.master_id : null,
            masterName: event ? event.master_name : null,
            eventStatus: event ? event.event_status : null,
            createdAt: event ? event.created_at : null,
            closedAt: event ? event.closed_at : null,
          });
        }
        ...
        sizes.push({
          sizeId: size.size_id,
          sizeLabel: size.size_label,
          patternCount: size.pattern_count,
          totalPieces: size.total_pieces,
          bundleCount: size.bundle_count,
          totalBundles: bundles.length,
          assignedBundles: sizeAssigned,
          openBundles: sizeOpen,
          closedBundles: sizeClosed,
          bundles,
        });
      }
      ...
      console.error('[production-flow] lot preview error:', error);
      return res.status(500).json({ error: 'Failed to load lot details.' });
    } finally {
      connection.release();
    }
  },
